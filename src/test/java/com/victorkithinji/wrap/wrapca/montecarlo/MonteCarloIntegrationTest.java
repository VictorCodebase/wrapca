...
import com.victorkithinji.wrap.wrapca.simulation.IgnitionProbabilityResolver;
...
class MonteCarloIntegrationTest {
	...
	void setUp() {
		...
		// Wire CaSpreadEngine with real dependencies
		IgnitionProbabilityResolver ignitionResolver = new IgnitionProbabilityResolver(config);
		CaSpreadEngine engine = new CaSpreadEngine(ignitionResolver, config);
	...
	private CaGrid buildDryGrassGrid() {
		...
		for (int r = 0; r < ROWS; r++)
			for (int c = 0; c < COLS; c++) {
				envs[r][c] = new CellEnvironment(0.5f, DRY_NDMI, 0f, 0f, 0.0f, VegetationTypeEnum.GRASSLAND);
				states[r][c] = CellStateEnum.UNBURNED.ordinal();
			}
		return new CaGrid(states, envs, ROWS, COLS, CELL_SIZE);
	}
	...
	private CaGrid buildGridWithWaterCentre() {
		...
		envs[cr][cc] = new CellEnvironment(0f, 1f, 0f, 0f, 0.0f, VegetationTypeEnum.WATER);
